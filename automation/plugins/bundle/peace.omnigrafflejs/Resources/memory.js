...
(() => {
    ...
    library.drawMemoryBlocksAbstractly = function (canvas, startPoint, descriptions) {
        ...
        return new Group(
            descriptions.map((description, index) => {
                ...
                return this.drawMemoryRect(canvas, startPoint, description);
            })
        );
    }
    ...
    library.drawStackMemoryAbstractly = function (canvas, origin) {
        ...
        this.setStyle('small');
        this.dynamic.labelTextBase = 10;
        this.dynamic.showSize = false;
        let common = this.plugIn.library("common");
        canvas = canvas || common.canvas();
        origin = origin || common.windowCenterPoint();
        common.readFileContentForGraphic(canvas, "location")
            .then(responce => {
    ...
    library.drawMemoryRect = function (canvas, location, description) {
        ...
        shape.geometry = new Rect(location.x, location.y, dynamic.rectSize.width, dynamic.rectSize.height);
    ...
    library.drawMemoryAddress = function (canvas, origin, address) {
        ...
        label.textHorizontalAlignment = HorizontalTextAlignment.Left;
    ...
    library.drawMemorySize = function (canvas, location, size) {
        ...
        label.shadowColor = null;
        label.strokeThickness = 0;
        label.text = formatMemorySize(size);
        label.textSize = 12;
        label.fillColor = null;
